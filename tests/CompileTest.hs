...
import Prelude
import qualified Language.Actions as LA
...
pacmakeOptions = PSCMakeOptions {
  pscmInput = ["tests/data/Main.purs","tests/data/Prelude.purs"] ,
  pscmOutputDir = "tests/data/output" ,
  pscmOpts = P.Options False False (S.fromList [P.CoreFn]) ,
  pscmUsePrefix = False,
  pscmJSONErrors = False
                                }

test = compile pacmakeOptions
...
compile PSCMakeOptions{..} = do
  ...
  (makeErrors, makeWarnings) <- runMake pscmOpts $ do
    ...
    let makeActions =LA.buildMakeActions pscmOutputDir filePathMap foreigns pscmUsePrefix
