...
-spec start_link(Module, Args) -> startlink_ret() when
      ...
      Args :: term().
start_link(Mod, Args) ->
    gen_server:start_link(?SUP, {self, Mod, Args}, []).
 
-spec start_link_with(SupName, Module, Args) -> startlink_ret() when
      SupName :: sup_name(),
      Module :: module(),
      Args :: term().
start_link_with(SupName, Mod, Args) ->
    gen_server:start_link(SupName, ?SUP, {SupName, Mod, Args}, []).
